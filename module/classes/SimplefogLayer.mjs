...
import MaskLayer from "./MaskLayer.mjs";
import { Layout } from "../libs/hexagons.mjs";
import {
    hexObjsToArr,
    hexToPercent,
    simplefogLog,
...
export default class SimplefogLayer extends MaskLayer {
	  ...
	  _updateScene(scene, data) {
		    ...
		    if (hasProperty(data, `flags.${this.layername}.blurQuality`)) {
			      canvas[this.layername].blur.quality = this.getSetting(
                "blurQuality"
            );
		    }
		    // React to composite history change
		    ...
		    if (
            !game.user.isGM &&
            hasProperty(data, `flags.${this.layername}.fogImageOverlayPlayerAlpha`)
        ) {
			      canvas[this.layername].setFogImageOverlayAlpha(
	  ...
	  setActiveTool(tool) {
		    ...
		    this.setPreviewTint();
		    const currentTool = $("#brush-size-container");
	  ...
	  _pointerDown(e) {
		    ...
		    if (e.data.button === 0) {
			      ...
			      // Call _pointermove so single click will still draw brush if mouse
            // does not move
			      this._pointerMove(e);
		    }
	  ...
	  _pointerMoveBrush(p) {
		    ...
		    if (this.op) {
			      // Send brush movement events to renderbrush to be drawn and added
            // to history stack
			      this.renderBrush({
				        shape: this.BRUSH_TYPES.ELLIPSE,
				        x: p.x,
	  ...
	  _pointerDownPolygon(p) {
		    ...
		    if (this.polygon.length) {
			      ...
			      if (
                xo < this.DEFAULTS.handlesize &&
                yo < this.DEFAULTS.handlesize
            ) {
	  ...
	  _pointerMoveGrid(p) {
		    ...
		    // Hex Grid
			      ...
			      const vertices = this.gridLayout.polygonCorners({
                q: qr.q,
                r: qr.r
            });
