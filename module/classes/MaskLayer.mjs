...
export default class MaskLayer extends InteractionLayer {
    ...
    static get layerOptions() {
        return mergeObject(super.layerOptions, {
            ...
            // Ugly hack - render at very high zindex and then re-render at
            // layer init with layerZindex value
            zIndex: game.settings.get("simple-fog", "zIndex")
        });
    }
    ...
    /**
     * Called on canvas init, creates the canvas layers and various objects and
     * registers listeners.
     *
     * Important objects:
     *
     ...
     * fogImageOverlayLayer - PIXI Sprite that holds the image applied over the
     *                        fog color
     */
    initMask() {
        ...
        // Apply image overlay to fog layer after we renderStack to prevent
        // revealing the map
        this.fogImageOverlayLayer = new PIXI.Sprite();
        this.fogImageOverlayLayer.position.set(
            canvas.dimensions.sceneRect.x,
            canvas.dimensions.sceneRect.y
        );
    ...
    // Tint & Alpha have special cases because they can differ between GM &
    // Players and alpha can be animated for transition effects
    ...
    /* -------------------------------------------- */
    async setFogImageOverlayTexture(
        fogImageOverlayFilePath = this.getSetting("fogImageOverlayFilePath")
    ) {
     ...
     * Some properties have different values depending on if user is a GM or
     * player
     */
     ...
     * @param start {Number}        The position in the history stack to begin
     *                              rendering from
     * @param stop {Number}         The position in the history stack to stop
     *                              rendering
     */
    renderStack(
        ...
        if (history === undefined && !isInit) {
            simpleFogLogDebug(
                "MaskLayer.renderStack - set visible to autoEnableSceneFog"
            );
            this.visible = game.settings.get("simple-fog", "autoEnableSceneFog");
            return;
        }
    ...
    async commitHistory() {
        ...
        // If pointer is less than history length (f.x. user undo), truncate
        // history
    ...
    /**
     * Blanks out the mask of the layer
     */
    async blankMask() {
    ...
    brush(data) {
        ...
        const visible =
            typeof data.visible === "undefined" ? true : data.visible;
    ...
    /**
     * Gets a brush using the given parameters, renders it to mask and saves the
     * event to history
     * @param data {Object}       A collection of brush parameters
     * @param save {Boolean}      If true, will add the operation to the history
     *                            buffer
     */
    ...
    refreshZIndex() {
        canvas.simpleFog.zIndex = game.settings.get("simple-fog", "zIndex");
    }
}
