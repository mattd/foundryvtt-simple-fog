 ...
 */
import { dmToGM, simplefogLog, simplefogLogDebug } from "../js/utils.mjs";
...
export default class SimplefogMigrations {
    static check() {
        ...
        if (ver === 1) {
            ...
            SimplefogMigrations.migration1();
        }
    }
            ...
            if (s.data.flags?.simplefog?.history?.events) {
                ...
                for (let i = 0; i < history.events.length; i += 1) {
                    for (let j = 0; j < history.events[i].length; j += 1) {
                        ...
                        if (history.events[i][j].x) {
                            history.events[i][j].x = Math.round(
                                history.events[i][j].x
                            );
                        }
                        if (history.events[i][j].x) {
                            history.events[i][j].y = Math.round(
                                history.events[i][j].y
                            );
                        }
                        if (history.events[i][j].height) {
                            history.events[i][j].height = Math.round(
                                history.events[i][j].height
                            );
                        }
                        if (history.events[i][j].width) {
                            history.events[i][j].width = Math.round(
                                history.events[i][j].width
                            );
                        }
                        // Remove unnecessary visible & alpha props
                        if (history.events[i][j].visible) {
                            delete history.events[i][j].visible;
                        }
                        if (history.events[i][j].alpha) {
                            delete history.events[i][j].alpha;
                        }
                    }
                }
        ...
        game.scenes.forEach(async s => {
            ...
            if (s.flags?.simplefog?.gmAlpha) {
                s.setFlag(
                    "simplefog",
                    "gmColorAlpha",
                    s.data.flags.simplefog.gmAlpha
                );
                ...
                s.setFlag(
                    "simplefog",
                    "gmColorAlpha",
                    canvas.simplefog.DEFAULTS.gmColorAlpha
                );
            }
            ...
            if (s.flags?.simplefog?.gmTint) {
                ...
                s.setFlag(
                    "simplefog",
                    "gmColorTint",
                    canvas.simplefog.DEFAULTS.gmColorTint
                );
            }
            ...
            if (s.flags?.simplefog?.playerAlpha) {
                s.setFlag(
                    "simplefog",
                    "playerColorAlpha",
                    s.data.flags.simplefog.playerAlpha
                );
                await s.unsetFlag("simplefog", "playerAlpha");
            } else if (
                s.getFlag("simplefog", "playerColorAlpha") === undefined
            ) {
                s.setFlag(
                    "simplefog",
                    "playerColorAlpha",
                    canvas.simplefog.DEFAULTS.playerColorAlpha
                );
            }
            ...
            if (s.flags?.simplefog?.playerTint) {
                s.setFlag(
                    "simplefog",
                    "playerColorTint",
                    s.data.flags.simplefog.playerTint
                );
                await s.unsetFlag("simplefog", "playerTint");
            } else if (
                s.getFlag("simplefog", "playerColorTint") === undefined
            ) {
                s.setFlag(
                    "simplefog",
                    "playerColorTint",
                    canvas.simplefog.DEFAULTS.playerColorTint
                );
            }
            ...
            if (s.flags?.simplefog?.layerZindex) {
                s.setFlag(
                    ...
                    s.data.flags.simplefog.layerZindex
                );
                await s.unsetFlag("simplefog", "layerZindex");
            } else if (
                s.getFlag("simplefog", "fogImageOverlayZIndex") === undefined
            ) {
                s.setFlag(
                    "simplefog",
                    "fogImageOverlayZIndex",
                    canvas.simplefog.DEFAULTS.fogImageOverlayZIndex
                );
            }
            ...
            if (s.flags?.simplefog?.fogTextureFilePath) {
                s.setFlag(
                    ...
                    s.data.flags.simplefog.fogTextureFilePath
                );
                await s.unsetFlag("simplefog", "fogTextureFilePath");
            }
        });
        dmToGM(
            game.i18n.localize("SIMPLEFOG.migration2Notification"),
            undefined
        );
